...
#
# Tags look like `v<Docker Registry Purger version>-<release>`:
#
# - if defaults/main.yml points at a Docker Registry Purger version that has
#   never been released, the release counter restarts at 0 (`v1.1.0-0`)
# - otherwise the counter is incremented (`v1.0.0-10`), but only if something
...
#
# `docker_registry_purger_version` is the plain literal that Renovate's
# annotation in defaults/main.yml is attached to, and the one that the
# container image reference is built from. A variable that merely interpolates
# it (`docker_registry_purger_container_image_tag`, and so on) is never
# rewritten by Renovate, and reading one would leave this printing Jinja rather
# than a version.
...
version="$(sed -nE 's|^docker_registry_purger_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Docker Registry Purger version from $defaults_path"
	exit 1
fi

# The version values do not carry a leading `v` (e.g. `1.0.0`), but the tags
# do (`v1.0.0-0`). Stripping any `v` before prepending one keeps this correct
